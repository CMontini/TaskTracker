...
export async function GET(req:Request){try{const owner=ownerOf(req),db=database();await db.batch([['classes','Classes','#5269dd','Classes'],['general','General','#1f9b85','General'],['personal','Personal','#c4588a','Personal']].map(([key,name,color,category])=>db.prepare('INSERT OR IGNORE INTO subjects(id,owner,name,color,category) VALUES(?,?,?,?,?)').bind(owner+':'+key,owner,name,color,category)));return Response.json(await state(owner),{headers:{'Cache-Control':'no-store'}});}catch(e){return fail(e);}}
