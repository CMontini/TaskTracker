...
export type Task={id:string;title:string;notes:string;subjectId:string;type:string;priority:typeof priorities[number];status:'todo'|'progress'|'done';dueDate:string|null;dueTime:string|null;repeat:typeof repeats[number];interval:number;anchorDay:number|null;completedAt:string|null;createdAt:string;version:number;parentId:string|null;archivedAt:string|null};
export type Subject={id:string;name:string;color:string;category:string;archivedAt:string|null;version:number};
